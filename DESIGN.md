...
The Send screen splits vertically into two regions:
...
- Wrap the screen body in `<KeyboardAvoidingView behavior="padding">` (iOS) so the bottom block lifts as the keyboard rises rather than overlapping it.
- The top region is a flex-1 `<ScrollView>` that occupies whatever space is left above the action zone.
- The bottom region (`thumbZone`) is a sibling `<View>` with no flex, so its height is determined by its content (card + button).
- Apply `paddingBottom: max(insets.bottom, space.s3)` on the action zone so it clears the iOS home indicator when the keyboard is closed; the safe-area gap is automatically swallowed by the keyboard padding when the keyboard is open.
- Top of the thumb zone gets a 2px black hairline (`borderTopWidth: 2`, `borderTopColor: palette.black`) — a Brutalist seam between the recipient region above and the action zone below.

### Brutalist Overrides (Send Screen Only)

The base design system uses a 12px card radius and 50px full-pill buttons. The Send surface overrides those tokens with strict angular geometry to align with the "shot" theme:
...
The override is local to the Send surface — Bank, Profile, and other tabs continue to use the standard pill/card tokens.
...
**Frames.** `translateY` values scale to `useWindowDimensions().height` (`screenH`) so the bullet exits the top edge on every device size:

| Frame | Phase | translateY | scaleX | scaleY | Hold |
|-------|-------|-----------:|-------:|-------:|-----:|
| 0 | Idle | follows finger drag (clamped −120…0) | 1.0 → 0.96 | 1.0 → 1.06 | — |
| 1 | Chambering (squash) | +6 | 1.06 | 0.84 | 80 ms |
| 2 | Transformation (collapse to bullet) | −20 | 0.18 | 0.46 | 60 ms — `onShoot()` fires here |
| 3 | Shot leap 1 | `−0.30 × screenH` | 0.16 | 0.40 | 60 ms |
| 4 | Shot leap 2 | `−0.65 × screenH` | 0.14 | 0.32 | 60 ms |
| 5 | Shot leap 3 (bullet at top edge) | `−0.95 × screenH` | 0.12 | 0.26 | 60 ms |
| 6 | Exit (off-screen) | `−1.30 × screenH` | 0.08 | 0.16 | 80 ms → reset |

**Trigger thresholds.** The pan gesture fires when `translationY < −60` *or* `velocityY < −800`. If `canShoot` is false at trigger time the card snaps back and a warning haptic fires; the parent shows the appropriate "type a word" / "pick a friend" alert.

**Side-effect timing.** The Firestore `sendWord(...)` call is dispatched at the Frame 2 → Frame 3 boundary via `runOnJS(onShoot)` from the worklet callback. The parent does an optimistic clear of `word` / `note` / `recipient` synchronously inside `onShoot`, so when the bullet exits and the card resets to Frame 0 the input is already blank for the next word. Errors surface via `Alert` after the animation has already played.
...
A `Gesture.Pan()` detector is attached to each row with `activeOffsetX([-15, 15])` and `failOffsetY([-15, 15])`. The fail-offset lets the FlatList's vertical scroll claim ownership first if the user starts swiping vertically; a clearly horizontal drag (`> 15px`) activates the row gesture instead. Once activated, the drag is clamped to `±120px`.

**Commit thresholds** — the gesture commits when `|translationX| > 60` *or* `|velocityX| > 800`. Below that, the card snaps back with a `120ms` spring. While dragging, a swipe-direction affordance (`SAVE` in green / `DISCARD` in red) fades in behind the card at `80px` opacity full-scale, giving the user a preview of the action.

**Haptics** — save triggers `ImpactFeedbackStyle.Medium`; discard triggers `ImpactFeedbackStyle.Light`.

### Save Animation — Right Swipe

The card transforms into an angular bullet and arcs toward the Bank tab at the bottom of the screen. Total duration ≈ 265 ms.

| Frame | Phase | translateX | translateY | scaleX | scaleY | Skin | Hold |
|-------|-------|------------|------------|--------|--------|------|------|
| 0 | Idle | follows finger (clamped ±120) | 0 | 1.015 max | 0.975 min | card | — |
| 1 | Slide right | `+0.42 × screenW` | 0 | 1.0 | 1.0 | card | 55 ms |
| 2 | Compress | `+0.52 × screenW` | 0 | 0.22 | 0.22 | **bullet square** ← `onSave()` fires here | 60 ms |
| 3 | Fly to Bank tab | `+0.08 × screenW` | `+0.5 × screenH` | 0.07 | 0.07 | bullet square | 150 ms |
| 4 | Done | — | — | — | — | opacity 0 | — |

The "bullet square" is a `28×28px` solid black `View` with `borderRadius: 0`, centered over the card via `StyleSheet.absoluteFillObject`. The card skin (word, note, sender meta) cross-fades to opacity `0` as the bullet appears at Frame 2.

**Side-effect timing.** `onSave()` is dispatched via `runOnJS` at the Frame 2 → Frame 3 boundary (as the bullet begins to fly). The parent immediately filters the card from local state (optimistic) and calls `decideCard(id, 'saved')` async.

### Discard Animation — Left Swipe

The card snaps left and shatters into three angular shards that scatter outward before fading. Total duration ≈ 205 ms.

| Frame | Phase | Card | Fragments | Hold |
|-------|-------|------|-----------|------|
| 0 | Idle | follows finger | hidden | — |
| 1 | Slide left | `translateX −0.38 × screenW`, full card visible | hidden | 55 ms |
| 2 | Shatter | opacity 0 | appear at positions near the slid card center, scattered 18–30 px, rotated ±14°–20° | 75 ms |
| 3 | Fade | opacity 0 | scatter further (42–72 px), rotate ±26°–38°, opacity 0.3 | 75 ms |
| 4 | Done | — | opacity 0 — `onDiscard()` fires | — |

**Fragment geometry.** Three `Animated.View` elements with `position: absolute` fill the `rowWrap`. Each contains a skewed black rectangle (`borderRadius: 0`, `skewX: ±4°–8°`) representing a torn card piece. Their `translateX` values are anchored to `−screenW × 0.35` (the card's center during the slide frame) and offset from there for scatter. The three shards simulate an upper-left chunk (skewX −8°), an upper-right chunk (skewX +6°), and a lower-center strip (skewX −4°).

**Side-effect timing.** `onDiscard()` fires at the Frame 3 → Frame 4 boundary via `runOnJS`. The parent filters the card from local state and calls `decideCard(id, 'discarded')` async.

### Empty State

When the inbox is empty, a centred message — **"No new shots."** in `bodyLarge` / `textBlackSoft` — fills the list area. No action is offered; the state resolves when a friend sends a word. The heading `h1 "Incoming"` and subtitle ("All clear" / "N new shots") remain visible in the header at all times.
